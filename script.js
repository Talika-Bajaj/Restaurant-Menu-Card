const menu = [
  ...
  { id: 14, title: "Pizza Margherita", category: "mains", price: "$25", description: "Pizza Margherita is a typical Neapolitan pizza with a raised edge and garnished with tomatoes, mozzarella and fresh basil leaves.", img: "https://upload.wikimedia.org/wikipedia/commons/4/41/Margherita_-_Five50_Aria.jpg" },
  { id: 15, title: "Alfredo Penne Pasta", category: "starters", price: "$13", description: "Alfredo penne pasta is pasta tossed with butter, Parmesan cheese to form a rich creamy sauce with brocolli, carrot, mushrooms", img: "https://lilluna.com/wp-content/uploads/2022/11/white-sauce-pasta-resize-10.jpg" },
];
...
menu.forEach((item) => {
  menuDishes.innerHTML += `<div class="menu-item col-12 col-md-6 col-lg-4"  style = "width: 20rem; box-shadow: 1px 0px 6px 6px #ddd;background-color: #ddd; cursor: pointer">
        <div class="card">
          <img src="${item.img}" class="card-img-top" alt="${item.title}" style = "height: 200px">
          <div class="card-body">
            <h5 class="card-title">${item.title}</h5>
            <h6 class="card-subtitle mb-2 text-muted">${item.price}</h6>
            <p class="card-text" style = "color: firebrick">${item.description}</p>
  ...
  menu.forEach((item) => {
    menuDishes.innerHTML += `<div class="menu-item col-12 col-md-6 col-lg-4"  style = "width: 20rem; box-shadow: 1px 0px 6px 6px #ddd;background-color: #ddd; cursor: pointer">
        <div class="card">
          <img src="${item.img}" class="card-img-top" alt="${item.title}" style="height: 200px">
            <div class="card-body">
              <h5 class="card-title" style="color: teal">${item.title}</h5>
              <h6 class="card-subtitle mb-2 text-muted">${item.price}</h6>
              <p class="card-text" style="color: firebrick">${item.description}</p>
              <p class="card-text"><small class="text-muted">Category: ${item.category}</small></p>
            </div>
...
//event handler for starters
let starters = document.getElementById("starters");
let startersItems = menu.filter(item => item.category === "starters");
...
let mainsItems = menu.filter(item => item.category === "mains");
...
//event handler for desserts
let desserts = document.getElementById("desserts");
let dessertItems = menu.filter(item => item.category === "desserts");
...
//event handler for beverages
let beverages = document.getElementById("beverages");
let beverageItems = menu.filter(item => item.category === "beverages");
